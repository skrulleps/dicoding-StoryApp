  ...
  afterRender() {
    ...
    this.savedStoriesContainer.innerHTML = stories.map(story => {
      ...
      <div class="story-card" data-id="${story.id}" style="border: 1px solid #ccc; border-radius: 8px; padding: 12px; box-sizing: border-box;">
        ...
        <button class="delete-saved-story-button" data-id="${story.id}" style="background-color: #e74c3c; color: white; border: none; padding: 6px 10px; border-radius: 4px; cursor: pointer; margin-top: 8px; font-size: 0.875rem;">Delete</button>
      </div>
      `;
    }).join('');
